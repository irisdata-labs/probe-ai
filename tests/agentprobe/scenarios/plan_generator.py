...
2. **dimensions**: The key variables in the world state that affect agent behavior. For each:
   - name: short identifier (snake_case)
   - category: grouping (e.g., "order", "customer", "product", "external_data")
   - description: what this variable represents
   - value_spec: type and possible values
     - type: "string", "int", "float", "bool", or "enum"
     - values: list of possible values (for enum)
...
3. **rubric**: Evaluation dimensions for LLM-as-judge assessment. For each:
...
Identify ALL dimensions that could affect agent behavior. Think about what
a QA team would need to test comprehensively.
